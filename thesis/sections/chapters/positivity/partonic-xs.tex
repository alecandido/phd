...
After the subtraction \cref{eq:pos/renorm}, the partonic cross-section
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{\small Mellin-space NLO contributions to deep-inelastic coefficient
      ...
      scheme is defined in \crefrange{eq:pos/counthqq}{eq:pos/counthqg},
      and the MPOS scheme
...
\end{figure}
  ...
  $C_q$ and $C_g$ are shown in \cref{fig:dis} in Mellin space,
  ...
  It is clear  from \cref{fig:dis} that at
...
\cref{sec:diag} below. Hence, while it has the same origin, this
...
in \cref{fig:dis}, and it is manifestly positive.
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{\small Mellin-space NLO contributions to Drell--Yan coefficient
      ...
      scheme is defined in \crefrange{eq:pos/counthqq}{eq:pos/counthqg}
      and \crefrange{eq:pos/counthgg}{eq:pos/counthgq}, and the MPOS scheme
      in \crefrange{eq:pos/mposqq}{eq:pos/mposgg}).
...
\cref{fig:dis}. It is clear that the coefficient function is
...
In order to fully characterize the scheme it is necessary to also consider
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{\small Same as \cref{fig:dy}, but now for the Higgs
...
in the DPOS scheme. This is seen in \cref{fig:dis} (right), where 
...
\cref{sec:offdiag}, but now for the quark coefficient function
...
\cref{fig:higgs}, is seen to be negative at large $N$.
...
shown in \cref{fig:higgs} (left), so no further scheme change is needed.
...
\Crefrange{eq:pos/counthqq}{eq:pos/counthqg} and
\crefrange{eq:pos/counthgg}{eq:pos/counthgq} fully define the POS subtraction.
