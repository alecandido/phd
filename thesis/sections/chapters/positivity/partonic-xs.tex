...
Eqs.~(\ref{eq:pos/refaca}-\ref{eq:refacb}) the
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{\small Mellin-space NLO contributions to deep-inelastic coefficient
      ...
      shown. The DPOS
      scheme is defined in Eqs.~(\ref{eq:pos/count},\ref{eq:countq}), the POS
      scheme is defined in Eqs.~(\ref{eq:pos/counthqq}-\ref{eq:counthqg}),
      and the MPOS scheme
      in Eqs.~(\ref{eq:pos/mposqq}-\ref{eq:mposqg}).
...
Comparing  Eqs.~(\ref{eq:pos/cgeps}-\ref{eq:cgren}) immediately reveals
...
counterparts Eqs.~(\ref{eq:pos/cgren},\ref{eq:cgrenpos}) in terms of
...
\begin{figure}[t]
  \begin{center}
    ...
    \caption{\small Mellin-space NLO contributions to Drell--Yan coefficient
      ...
      scheme is defined in Eqs.~(\ref{eq:pos/counthqq}-\ref{eq:counthqg})
      and (\ref{eq:pos/counthgg}-\ref{eq:counthgq}), and the MPOS scheme
      in Eqs.~(\ref{eq:pos/mposqq}-\ref{eq:mposgg}).
...
\end{align}
Equations~(\ref{eq:pos/count},\ref{eq:countq}) thus define the DPOS
...
Eq.~(\ref{eq:pos/dyq}-\ref{eq:dyg}) to their DIS counterparts
Eqs.~(\ref{eq:pos/cgren},\ref{eq:cqnlo}) shows that they have the same
...
where $ Q^2 = M^2 $.
The coefficient
functions, Eq.~(\ref{eq:pos/dyq}-\ref{eq:dyg}), are displayed in
...
Eqs.~(\ref{eq:pos/dyg},\ref{eq:counthqg}) and it is easy to check that it
...
channel for DIS, Eqs.~(\ref{eq:pos/cgeps},\ref{eq:cgren}): the collinear
...
Equations~(\ref{eq:pos/counthqq}-\ref{eq:counthqg}) and (\ref{eq:counthgg}-\ref{eq:counthgq}) 
