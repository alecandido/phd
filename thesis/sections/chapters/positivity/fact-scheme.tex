...
Eqs.~(\ref{eq:refaca}-\ref{eq:refacb}), and, very crudely speaking, amounts to
showing that with the POS subtraction, all factors in Eqs.~(\ref{eq:refacb})
...
the nonsinglet case the DIS structure function Eq.~(\ref{eq:f2}) becomes
...
The factorization Eqs.~(\ref{eq:refaca}-\ref{eq:refacb}) takes the
form
\begin{align}
...
Eq.~(\ref{eq:distomsbarllx}).\footnote{
  ...
  term in square brackets in Eq~(\ref{eq:distomsbarllx}) starts with one.
...
\begin{equation}\label{eq:singhadfact}
 ...
 \end{equation}
In Eq.~(\ref{eq:singhadfact}) \begin{itemize}
...
It is worth noting that a consequence of Eq.~(\ref{eq:dglap}) is that,
...
Inspection of Eq.~(\ref{eq:schch}) immediately shows a possible issue
