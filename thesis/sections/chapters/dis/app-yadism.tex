...
over an interpolation basis \begin{equation}
...
%
In \yadism the polynomial interpolation basis is  provided by \textsc{\small EKO}.
%
...
The grid formalism summarised schematically in Eq.~(\ref{app/dis:grid_formalism})
...
\begin{figure}[!t]
 \centering
 \includegraphics[width=0.48\linewidth]{plots/matched_datasets_from_dataspecs14_dataset_report_Datanorm_plot_fancy_dataspecs_11.pdf}
 \includegraphics[width=0.48\linewidth]{plots/matched_datasets_from_dataspecs14_dataset_report_Datanorm_plot_fancy_dataspecs_23.pdf}
 \includegraphics[width=0.48\linewidth]{plots/matched_datasets_from_dataspecs4_dataset_report_Datanorm_plot_fancy_dataspecs_0.pdf}
 \includegraphics[width=0.48\linewidth]{plots/matched_datasets_from_dataspecs2_dataset_report_Datanorm_plot_fancy_dataspecs_0.pdf}
 \caption{Comparison of the \textsc{\small YADISM} predictions for DIS structure functions
   and reduced cross-sections
   at NNLO with the corresponding ones from \textsc{\small APFEL} for the same choice of input settings.
   %
   We display  predictions for four $x$ bins of representative DIS datasets included
   in the NNPDF4.0 global analysis: fixed-target neutral-current DIS on a deuteron target
   from BCDMS, fixed-target charged-current DIS on a lead target
   from CHORUS, collider neutral-current positron-proton DIS from HERA,
   and collider charged-current electron-proton DIS from HERA.}    
