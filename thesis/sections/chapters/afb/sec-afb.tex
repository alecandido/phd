...
distributions \cref{eq:dsigma-dcos} and the
forward-backward asymmetry \cref{eq:forward-backward-asymmetry}, with
...
the discussion of the \pdf features from \cref{sec:largexpdfs},
we also provide results for the two partonic channels that give the
largest contribution to the cross-section.
%
As in \cref{sec:largexpdfs}, we compare results obtained using
...
\nnpdfr{4.0} result, \cref{eq:pulldef_xsec} (bottom).
%
...
\cref{fig:CMS_DY_14TEV_MLL_5000_rap} we display the 
differential distribution in absolute dilepton rapidity $|\yll|$,
defined in \cref{eq:dsigma-dyll},
...
As discussed in \cref{sec:HMDY}, the  $|y_{\ell\bar{\ell}}|$  distribution
depends  on the symmetric partonic luminosities $\mathcal{L}_{S,q}$, \cref{eq:lumiss_qpm},
...
\caption{Same as \cref{fig:CMS_DY_14TEV_MLL_5000_rap}, now for the differential distribution in
  $\cos\theta^*$ (left)
...
120$~GeV, in \cref{fig:CMS_DY_14TEV_MLL_zpeak}.
%
The $\cos\theta^*$ 
...
\cref{subsec:partoniclumis}, the situation is quite different when
...
in the high-mass region, for different values of the  lower cut in the dilepton
 ...
 respectively
 shown in
\cref{fig:CMS_DY_14TEV_MLL_others} and \cref{fig:CMS_DY_14TEV_MLL_others_asy}.
...
\begin{figure}[t!]
 ...
 \caption{Same as \cref{fig:CMS_DY_14TEV_MLL_zpeak} (left)
...
%-------------------------------------------------------------------------------
 ...
 \cref{sec:subsec-largexPDFs}-\ref{subsec:partoniclumis}.
%
 Specifically,
 raising the cut to
 $\mll \ge 4$ TeV, for \nnpdfr{4.0}
...
\begin{figure}[t!]
 ...
 \caption{Same as \cref{fig:CMS_DY_14TEV_MLL_zpeak} (right)
   for different values of the  lower cut in the dilepton
